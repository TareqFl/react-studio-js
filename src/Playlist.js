...
// ==========================================>
    ...
    ee.on("shift", (deltaTime, track) => {
      ...
      this.adjustDuration();
      this.drawRequest();
      // emit shifted track to be updated
      ee.emit('shiftedTrack', {query:track.id,startTime:track.startTime})
    });
    ...
    ee.on("fadein", (duration, track) => {
      ...
      ee.emit('saveTracksToLocalStorage', {tracks:this.tracks, stateName:'fadein'})
      this.drawRequest();
    ...
    ee.on("fadeout", (duration, track) => {
      ...
      ee.emit('saveTracksToLocalStorage', {tracks:this.tracks, stateName:'fadeout'})
      this.drawRequest();
  ...
  */
  ...
  async startOfflineRender(type) {
    ...
    // adjust channels and samples pers second to have higher audio quality
  ...
  removeTrack(track) {
    ...
    this.adjustDuration();
    // emit tracks in order to know if theres available tracks to disable or enable UI buttons
    this.ee.emit('tracksLeft', this.tracks.length)
    // emit track duration
